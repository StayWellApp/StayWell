...
// Replace the entire contents of your ClientDashboard.js file with this code.

import React, { useState, useEffect } from 'react';
import { db } from '../firebase-config';
import { collection, query, where, onSnapshot } from 'firebase/firestore';
import { BarChart, Bar, XAxis, YAxis, Tooltip, Legend, ResponsiveContainer, PieChart, Pie, Cell } from 'recharts';
import { DollarSign, Bed, CalendarCheck, ClipboardList, AlertTriangle } from 'lucide-react';
...
const ClientDashboard = ({ user }) => {
    ...
    return (
        <div className="p-8 bg-gray-50">
            <h1 className="text-3xl font-bold text-gray-900">Welcome back, {user.displayName || 'Host'}!</h1>
            <p className="text-gray-600 mt-1">Here's a snapshot of your property management operations.</p>

            {/* Stats Cards */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
                <StatCard title="Total Properties" value={properties.length} icon={<Bed size={24} className="text-purple-600"/>} color="bg-purple-100" />
                <StatCard title="Upcoming Bookings" value="7" icon={<CalendarCheck size={24} className="text-green-600"/>} color="bg-green-100" />
                <StatCard title="Pending Tasks" value={tasksByStatus['Pending'] || 0} icon={<ClipboardList size={24} className="text-yellow-600"/>} color="bg-yellow-100" />
                <StatCard title="Urgent Issues" value="2" icon={<AlertTriangle size={24} className="text-red-600"/>} color="bg-red-100" />
            </div>
