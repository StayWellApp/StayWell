...
// --- Pre-generated templates with more detailed instructions ---
const preGeneratedTemplates = [
    {
        ...
        linkedProperties: [],
        items: [
            { text: "Strip all beds and start laundry", instructions: "Include all sheets, pillowcases, duvet covers, and towels. Use the 'Whites' setting with hot water.", imageUrl: "" },
            { text: "Wash, dry, and put away all dishes", instructions: "Check the dishwasher first. Hand-wash any items that are not dishwasher-safe. Ensure the sink is empty and clean.", imageUrl: "" },
            { text: "Wipe down all kitchen counters and sink", instructions: "Use multi-surface cleaner. Pay special attention to the area around the faucet and sink drain.", imageUrl: "" },
            { text: "Clean and sanitize toilets, showers, and bathroom sinks", instructions: "Use appropriate bathroom cleaner. Scrub the toilet bowl. Wipe down the toilet seat, lid, and base.", imageUrl: "" },
            { text: "Dust all accessible surfaces", instructions: "This includes tables, nightstands, dressers, window sills, bookshelves, and picture frames.", imageUrl: "" },
            { text: "Vacuum all carpets and mop all hard floors", instructions: "Start from the farthest corner of the room and work your way towards the door.", imageUrl: "" },
            { text: "Restock all consumables", instructions: "Check levels of toilet paper (2 extra rolls per bathroom), paper towels (1 extra roll), hand soap, dish soap, and coffee pods (at least 10).", imageUrl: "" },
            { text: "Make all beds with fresh linens", instructions: "Ensure linens are wrinkle-free and pillows are fluffed. Place decorative pillows neatly.", imageUrl: "" },
        ]
    },
    {
        ...
        linkedProperties: [],
        items: [
            { text: "Clean inside of oven and microwave", instructions: "Use oven cleaner for the oven, following product directions. For the microwave, heat a bowl of water and lemon juice for 3 minutes to loosen grime before wiping.", imageUrl: "" },
            ...
            { text: "Wash all windows, inside and out", instructions: "Use a squeegee for a streak-free finish. A solution of water and a small amount of dish soap works well.", imageUrl: "" },
            { text: "Dust and wipe down all baseboards and trim", instructions: "A damp cloth works well for this task. Pay attention to corners where dust accumulates.", imageUrl: "" },
            { text: "Clean behind and under large furniture", instructions: "Carefully move sofa and beds to vacuum and mop underneath. Check for lost items.", imageUrl: "" },
        ]
    }
];
    ...
    return (
        <div className="p-4 sm:p-6 md:p-8">
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => { setEditingTemplate(null); setShowAddForm(true); }} className="button-primary flex items-center">
                    <Plus size={18} className="mr-2" />Create Template
                </button>
            </div>

            {showAddForm && <ChecklistTemplateForm onSave={handleSave} onCancel={() => setShowAddForm(false)} properties={properties} />}
                        ...
                        {templates.map(template => (
                            editingTemplate?.id === template.id ? (
                                <li key={template.id} className="py-3">
                                    <ChecklistTemplateForm onSave={handleSave} onCancel={() => setEditingTemplate(null)} existingTemplate={editingTemplate} properties={properties} />
                                </li>
                            ) : (
                                <TemplateItem 
                                    key={template.id}
            ...
            <div className="p-2 flex justify-between items-center">
                <div className="flex-grow cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                    <div className="flex items-center">
                        <ChevronDown size={20} className={`mr-2 text-gray-400 transition-transform ${isExpanded ? 'rotate-180' : ''}`} />
                ...
                <div className="space-x-4 flex-shrink-0">
                    <button onClick={onEdit} className="font-semibold text-blue-600 dark:text-blue-400 hover:underline">Edit</button>
                    <button onClick={onDelete} className="font-semibold text-red-600 dark:text-red-400 hover:underline">Delete</button>
                </div>
            </div>
            {isExpanded && (
                <div className="pl-10 pr-4 py-2 mt-2 bg-gray-50 dark:bg-gray-700/50 rounded-lg animate-fade-in-down">
                    <h4 className="font-semibold text-sm text-gray-700 dark:text-gray-300 mb-3">Checklist Items:</h4>
                    <ul className="space-y-3">
                        {template.items.map((item, index) => (
                            <li key={index} className="text-sm text-gray-800 dark:text-gray-200 border-l-2 pl-3 border-gray-300 dark:border-gray-600">
                                <p className="font-medium">{item.text}</p>
                                {item.instructions && <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{item.instructions}</p>}
                                {item.imageUrl && <img src={item.imageUrl} alt={item.text} className="mt-2 rounded-md max-h-32 border dark:border-gray-600" />}
                            </li>
                        ))}
                    </ul>
                </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex justify-end space-x-2 pt-4 border-t dark:border-gray-600">
                    ...
                    <button type="submit" className="bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-blue-700">Save Template</button>
                </div>
            </form>
        </div>
    );
