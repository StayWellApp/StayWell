    ...
    return (
        <div onClick={onClick} className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm cursor-pointer hover:shadow-md hover:-translate-y-1 transition-all">
            <div className="flex justify-between items-start">
                ...
                {task.recurring?.enabled && !task.recurring.isPrototype && <Repeat size={14} className="text-gray-400 flex-shrink-0" title={`Repeats ${task.recurring.frequency}`} />}
            </div>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{task.taskType}</p>
            {task.scheduledDate && <div className="mt-3 text-xs text-gray-500 dark:text-gray-400 flex items-center"><Calendar size={14} className="mr-1.5" /><span>Due: {new Date(task.scheduledDate + 'T00:00:00').toLocaleDateString()}</span></div>}
...
export const TasksView = ({ property, user }) => {
    ...
    useEffect(() => {
        ...
        const tasksUnsubscribe = onSnapshot(tasksQuery, (snapshot) => {
            const sortedTasks = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })).sort((a, b) => (a.scheduledDate > b.scheduledDate) ? 1 : -1);
            setTasks(sortedTasks);
            setLoadingTasks(false);
        }, (error) => { console.error("Error fetching tasks:", error); toast.error("Could not load tasks."); setLoadingTasks(false); });
        ...
        }, (error) => { console.error("Error fetching checklist templates:", error); toast.error("Could not load checklist templates."); });
        ...
        }, (error) => { console.error("Error fetching team members:", error); toast.error("Could not load team data."); });
    ...
    const activeTasks = useMemo(() => tasks.filter(task => !task.recurring?.isPrototype), [tasks]);
    const recurringPrototypes = useMemo(() => tasks.filter(task => task.recurring?.isPrototype), [tasks]);
    const filteredTasks = useMemo(() => activeTasks.filter(task => statusFilter === 'All' || task.status === statusFilter).filter(task => task.taskName.toLowerCase().includes(searchTerm.toLowerCase())), [activeTasks, statusFilter, searchTerm]);
    const groupedTasks = { Pending: filteredTasks.filter(t => t.status === 'Pending'), 'In Progress': filteredTasks.filter(t => t.status === 'In Progress'), Completed: filteredTasks.filter(t => t.status === 'Completed') };

    const FilterButton = ({ status }) => (<button onClick={() => setStatusFilter(status)} className={`px-4 py-2 text-sm font-semibold rounded-md transition-colors ${statusFilter === status ? 'bg-blue-500 text-white' : 'bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600'}`}>{status}</button>);

    return (
        <div className="flex flex-col h-full">
            <div className="flex-shrink-0">
                <div className="flex justify-between items-center mb-4"><h3 className="text-2xl font-semibold text-gray-800 dark:text-gray-100">Task Board</h3><div className="flex items-center space-x-2"><button onClick={() => setShowTemplateModal(true)} className="button-secondary"><ListChecks size={16} className="mr-2" /> Add from Template</button><button onClick={() => setShowAddTaskForm(true)} className="button-primary"><Plus size={18} className="mr-2" /> Add New Task</button></div></div>
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mb-6 p-4 bg-white dark:bg-gray-800 rounded-xl border dark:border-gray-700 shadow-sm"><div className="relative w-full md:w-1/3 mb-4 md:mb-0"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={20} /><input type="text" placeholder="Search tasks by name..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="input-style pl-10 w-full" /></div><div className="flex items-center space-x-2 p-1 bg-gray-100 dark:bg-gray-900 rounded-lg"><FilterButton status="All" /><FilterButton status="Pending" /><FilterButton status="In Progress" /><FilterButton status="Completed" /></div></div>
            </div>
            <div className="flex-grow min-h-0">
                {loadingTasks ? <p className="text-center py-8 text-gray-500 dark:text-gray-400">Loading tasks...</p> : (<div className="flex flex-col md:flex-row gap-6 h-full"><KanbanColumn title="Pending" tasks={groupedTasks.Pending} onTaskClick={setSelectedTask} /><KanbanColumn title="In Progress" tasks={groupedTasks['In Progress']} onTaskClick={setSelectedTask} /><KanbanColumn title="Completed" tasks={groupedTasks.Completed} onTaskClick={setSelectedTask} /></div>)}
            </div>
            ...
            {activatingTask && <ActivateRecurringTaskModal task={activatingTask} team={team} onActivate={handleActivateRecurringTask} onCancel={() => setActivatingTask(null)} />}
        </div>
    );
};