...
import { Calendar, User, CheckSquare, Trash2, Plus, MessageSquare, Siren, ListChecks, Info, Image, ChevronDown, Upload, Repeat, Edit, X } from 'lucide-react';
...
export const TaskDetailModal = ({ task, team, user, onClose }) => {
    ...
    useEffect(() => {
        ...
        const unsubscribeTask = onSnapshot(taskRef, (doc) => {
            if (doc.exists()) {
                setLiveTask({ id: doc.id, ...doc.data() });
...
const InlineSelect = ({ label, value, options, onChange }) => (
    <div>
        ...
        <select value={value} onChange={onChange} className="mt-1 w-full input-style text-sm py-1.5">
            {options.map(opt => (
                typeof opt === 'object' 
...
const InlineDate = ({ label, value, onChange }) => (
    <div>
        <label className="block text-xs font-medium text-gray-500 dark:text-gray-400">{label}</label>
        <input type="date" value={value} onChange={onChange} className="mt-1 w-full input-style text-sm py-1.5" />
    </div>
);
...
const TaskChecklist = ({ items, onToggle }) => {
    ...
    return (
        <div>
            ...
            <ul className="space-y-2">{items.map((item, index) => (<li key={index} className="bg-gray-50 dark:bg-gray-700/50 p-2 rounded-lg border dark:border-gray-600"><div className="flex items-center justify-between"><div className="flex items-center cursor-pointer" onClick={() => onToggle(index)}><input type="checkbox" checked={item.completed} readOnly className="w-5 h-5 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"/><span className={`ml-3 text-sm font-medium ${item.completed ? 'text-gray-500 dark:text-gray-400 line-through' : 'text-gray-900 dark:text-gray-100'}`}>{item.text}</span></div>{(item.instructions || item.imageUrl) && (<button onClick={() => setExpandedItem(expandedItem === index ? null : index)} className="p-1 text-gray-400 hover:text-gray-700 dark:hover:text-gray-200"><ChevronDown size={20} className={`transition-transform ${expandedItem === index ? 'rotate-180' : ''}`} /></button>)}</div>{expandedItem === index && (<div className="mt-2 pl-8 pr-4 py-2 bg-white dark:bg-gray-800 rounded-md animate-fade-in-down">{item.instructions && <p className="text-sm text-gray-600 dark:text-gray-300 flex items-start"><Info size={14} className="mr-2 mt-0.5 flex-shrink-0"/>{item.instructions}</p>}{item.imageUrl && (<div className="mt-2"><img src={item.imageUrl} alt="Instructional" className="rounded-lg max-w-xs max-h-48 border dark:border-gray-600" /></div>)}</div>)}</li>))}</ul>
        </div>
    );
};
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-xl w-full max-w-lg border dark:border-gray-700">
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <select value={selectedTemplateId} onChange={e => setSelectedTemplateId(e.target.value)} className="mt-1 w-full input-style">
                    ...
                    <div className="pt-4 border-t dark:border-gray-600 space-y-3">
                        <div className="flex items-center">
                            ...
                            <label htmlFor="recurring-template-checkbox" className="ml-2 block text-sm font-medium text-gray-900 dark:text-gray-200">Make this a recurring task</label>
                        </div>
                        {isRecurring && (
                            <div className="pl-6 animate-fade-in-down">
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Repeat every</label>
