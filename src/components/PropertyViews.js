...
import timeGridPlugin from '@fullcalendar/timegrid';
import interactionPlugin from '@fullcalendar/interaction';
import { Plus, Building, Bed, Bath, Users, Wifi, Tv, Wind, Utensils, Sun, Droplet, CookingPot, Flame, Tent, Bandage, Siren, CheckSquare, Calendar, BarChart2, Archive, Settings, Home, ListChecks, ChevronDown } from 'lucide-react';
import { BarChart as RechartsBarChart, Bar, XAxis, YAxis, Tooltip, Legend, ResponsiveContainer } from 'recharts';

// Main View Component
export const PropertyView = ({ user }) => {
    // ... state declarations ...
    ...
    const handleDeleteProperty = async (propertyId) => {
        if(window.confirm("Are you sure you want to delete this property and all associated data?")) {
    ...
    return (
        <div className="flex h-screen bg-gray-100 dark:bg-gray-900">
            <PropertySidebar
                ...
                onAddProperty={() => { setShowAddForm(true); setEditingProperty(null); }}
            />
            <main className="flex-1 p-6 overflow-auto">
                {showAddForm || showEditForm ? (
                    ...
                    <div>
                        ...
                        />
                        <div className="mt-4">
                            {view === 'dashboard' && <DashboardView property={selectedProperty} />}
                            {view === 'tasks' && <TasksView property={selectedProperty} />}
                            {view === 'calendar' && <CalendarView property={selectedProperty} />}
                            {view === 'checklists' && <ChecklistsView property={selectedProperty} />}
                            {view === 'inventory' && <InventoryView property={selectedProperty} />}
                        </div>
                    </div>
                ) : (
                    <div className="text-center py-20">
                        <h2 className="text-2xl font-semibold text-gray-700 dark:text-gray-300">Welcome to StayWell!</h2>
                        <p className="mt-2 text-gray-500 dark:text-gray-400">Select a property or add a new one to get started.</p>
                        <button onClick={() => setShowAddForm(true)} className="mt-6 button-primary">
                            <Plus className="mr-2" size={20} /> Add Your First Property
                        </button>
                    </div>
                )}
            </main>
...
export const PropertyForm = ({ onSave, onCancel, existingProperty = null }) => {
    ...
    useEffect(() => {
        if (existingProperty) {
            setPropertyName(existingProperty.propertyName || '');
            setPropertyType(existingProperty.propertyType || 'House');
            setAddress(existingProperty.address || '');
            setDescription(existingProperty.description || '');
            setBedrooms(existingProperty.bedrooms || 1);
            setBathrooms(existingProperty.bathrooms || 1);
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-8 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-6 animate-fade-in-down">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" htmlFor="propertyName">Property Name</label>
                        <input type="text" id="propertyName" value={propertyName} onChange={e => setPropertyName(e.target.value)} required className="input-text" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" htmlFor="propertyType">Property Type</label>
                        <select id="propertyType" value={propertyType} onChange={e => setPropertyType(e.target.value)} className="input-select">
                            <option>House</option>
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" htmlFor="address">Address</label>
                    <input type="text" id="address" value={address} onChange={e => setAddress(e.target.value)} required className="input-text" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" htmlFor="description">Description</label>
                    <textarea id="description" value={description} onChange={e => setDescription(e.target.value)} rows="3" className="input-textarea"></textarea>
                </div>
                <div className="grid grid-cols-3 gap-6">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" htmlFor="bedrooms">Bedrooms</label>
                        <input type="number" id="bedrooms" value={bedrooms} onChange={e => setBedrooms(e.target.value)} min="1" className="input-text" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" htmlFor="bathrooms">Bathrooms</label>
                        <input type="number" id="bathrooms" value={bathrooms} onChange={e => setBathrooms(e.target.value)} min="1" className="input-text" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" htmlFor="guests">Max Guests</label>
                        <input type="number" id="guests" value={guests} onChange={e => setGuests(e.target.value)} min="1" className="input-text" />
                    </div>
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Amenities</label>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                        {Object.entries(amenityOptions).map(([key, { icon: Icon, label }]) => (
                            <label key={key} className="flex items-center space-x-2 cursor-pointer p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">
                                <input type="checkbox" checked={!!amenities[key]} onChange={() => handleAmenityChange(key)} className="form-checkbox" />
                ...
                <div>
                    ...
                    {photoURL && <img src={photoURL} alt="Property" className="mt-4 w-32 h-32 object-cover rounded-lg" />}
                </div>
                <div className="flex justify-end space-x-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                    <button type="button" onClick={onCancel} className="button-secondary">Cancel</button>
                    <button type="submit" className="button-primary" disabled={isUploading}>{isUploading ? 'Uploading...' : (existingProperty ? 'Update Property' : 'Save Property')}</button>
                </div>
            </form>
        </div>
    );
...
// Other components... (PropertySidebar, PropertyHeader, DashboardView, etc.)
// These are not changed, but included for completeness.

const PropertySidebar = ({ properties, selectedProperty, onSelectProperty, onAddProperty }) => {
    return (
        <aside className="w-72 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col">
            <div className="p-4 border-b border-gray-200 dark:border-gray-700">
                <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100">My Properties</h2>
            </div>
            <nav className="flex-1 p-2 space-y-1 overflow-y-auto">
                {properties.map(prop => (
                    <a
                        key={prop.id}
                        href="#"
                        onClick={(e) => { e.preventDefault(); onSelectProperty(prop); }}
                        className={`flex items-center p-3 rounded-lg transition-colors ${selectedProperty?.id === prop.id ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300' : 'hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300'}`}
                    >
            ...
            <div className="p-4 mt-auto border-t border-gray-200 dark:border-gray-700">
                <button onClick={onAddProperty} className="button-primary w-full">
...
const PropertyHeader = ({ property, onNavigate, activeView, onEdit, onDelete }) => {
    const navItems = [
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">{property.propertyName}</h1>
                    <p className="text-gray-500 dark:text-gray-400">{property.address}</p>
                </div>
                <div className="relative">
                    <button onClick={() => setDropdownOpen(!dropdownOpen)} className="button-secondary">
                        Settings <ChevronDown size={20} className={`ml-2 transition-transform ${dropdownOpen ? 'rotate-180' : ''}`} />
                    </button>
                    {dropdownOpen && (
                        <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-700 rounded-md shadow-lg z-10">
                            <a href="#" onClick={(e) => {e.preventDefault(); onEdit(); setDropdownOpen(false);}} className="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600">Edit Property</a>
                            <a href="#" onClick={(e) => {e.preventDefault(); onDelete(); setDropdownOpen(false);}} className="block px-4 py-2 text-sm text-red-600 hover:bg-gray-100 dark:hover:bg-gray-600">Delete Property</a>
                        </div>
                    )}
                </div>
            </div>
                    ...
                    {navItems.map(item => (
                        <li key={item.id}>
                            <button
                                ...
                                className={`flex items-center px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeView === item.id ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'}`}
                            >
...
const DashboardView = ({ property }) => (
    <div className="p-4">
        <h3 className="text-xl font-bold mb-4">Dashboard for {property.propertyName}</h3>
        {/* Dashboard content goes here */}
    </div>
);
const TasksView = ({ property }) => (
    <div className="p-4">
        <h3 className="text-xl font-bold mb-4">Tasks for {property.propertyName}</h3>
        {/* Tasks content goes here */}
    </div>
);
const CalendarView = ({ property }) => (
    <div className="p-4">
        <h3 className="text-xl font-bold mb-4">Calendar for {property.propertyName}</h3>
        <FullCalendar
            plugins={[dayGridPlugin, timeGridPlugin, interactionPlugin]}
            initialView="dayGridMonth"
            headerToolbar={{
...
const ChecklistsView = ({ property }) => (
    <div className="p-4">
        <h3 className="text-xl font-bold mb-4">Checklists for {property.propertyName}</h3>
        <ChecklistTemplateForm propertyId={property.id} />
        {/* Checklists content goes here */}
    </div>
);