...
describe('respondToTaskOffer', () => {
  ...
  it('should successfully reject a task (primary -> fallback)', async () => {
    ...
    expect(result).toEqual({ success: true });
    expect(mockUpdate).toHaveBeenCalledWith(expect.objectContaining({ rejectionCount: expect.anything() }));
    expect(mockUpdate).toHaveBeenCalledWith(expect.objectContaining({ assignmentStatus: 'PendingFallback' }));
  });

  it('should successfully reject a task (fallback -> unassigned)', async () => {
    const data = { taskId, response: 'rejected' };
    const context = { auth: { uid: userId } };
